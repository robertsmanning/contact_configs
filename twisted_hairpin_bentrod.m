...
timings = zeros(1,2); avg_steps = zeros(1,2);
tic
...
% Shifting from soft- to hard-contact, adding bend, and interpolating up to nseg_final
nk = 6;
nsegvals = round(10.^(linspace(log10(nseg),log10(nseg_final),nk+1)));
for k = 1:nk
    par_ratio = cont_par_final/cont_par_init;
    K = cont_par_init*(par_ratio)^(1.0*k/nk);
    ...
    avgs_for_mer(:,4) = intrins_bend*(k/nk)/nseg;
    for i=1:nseg+1
      stiffs_for_mer(i,1:3) = [a1 a2 a3]*nseg;
      stiffs_for_mer(i,4:6) = [k1 k2 k3]*nseg;
    end
